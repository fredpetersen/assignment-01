...
public class RegExprTests
{
    ...
    [Fact]
    public void InnerText_when_given_nested_html_returns_all_inner_text()
    {
        ...
        result.Should().BeEquivalentTo("The phrase regular expressions (and consequently, regexes) is often used to mean the specific, standard textual syntax for representing patterns that matching text need to conform to.");
    }

    [Fact]
    public void Urls_when_given_html_with_links_returns_links_and_titles()
    {
        var html = "<div> in <a href=\"https://en.wikipedia.org/wiki/Theoretical_computer_science\" >theoretical computer science</a> and <a href=\"https://en.wikipedia.org/wiki/Formal_language\" title=\"Formal language\">formal language</a> theory </div>";

        var result = RegExpr.Urls(html);

        result.Should().BeEquivalentTo(new List<(Uri, string)>(){(new Uri("https://en.wikipedia.org/wiki/Theoretical_computer_science"),"theoretical computer science"),(new Uri("https://en.wikipedia.org/wiki/Formal_language"), "Formal language")});
    }
